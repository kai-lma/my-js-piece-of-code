...
const logScore = () =>
  console.log(
    Object.values($(INPUT_QUERY_SELECTOR))
      .filter(r => r.checked)
      .reduce((score, input, idx, arr) => (idx === arr.length - 1 ? (score + parseInt(input.value)) / arr.length : score + parseInt(input.value)), 0),
  );
...
const main = (() => {
  scoring();
  logScore();
  $(INPUT_QUERY_SELECTOR).on('click', logScore);
})();
